...
- run(): The infinite loop. It manages the elapsed_time counter.
- stop(): Uses a threading.Event to break the loop cleanly.

### B. SettingsDialog (extends simpledialog.Dialog)
This class manages user configuration.
- Centering Logic (center_dialog_on_screen):Standard tkinter dialogs often appear in random locations. This method calculates the precise center of the monitor:$$x = (Screen Width / 2) - (Window Width / 2)$$$$y = (Screen Height / 2) - (Window Height / 2)
